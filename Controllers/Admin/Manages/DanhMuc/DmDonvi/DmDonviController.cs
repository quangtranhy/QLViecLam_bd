...
namespace QLViecLam.Controllers.Admin.Manages.DanhMuc.DmDonvi
{
    public class DmDonviController : Controller
    {
        private readonly ApplicationDbContext _db;

        public DmDonviController(ApplicationDbContext db)
        {
            _db = db;
        }
            ...
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                ...
                if (check_per)
                {
                    var models = from dv in _db.DmDonvi
                                 join dmhc in _db.DmHanhChinh on dv.MaDiaBan equals dmhc.Id.ToString()
                    ...
                    ViewData["MenuLv2"] = "menu_quantrihethong_donvi";

                    return View("Views/Admin/Manages/TongHop_PhanTich_DuDoan/QuanTriHeThong/DmDonVi/Index.cshtml", models);
                }
                else
                {
        ...
        public IActionResult Store(string madv_create, string tendv_create, string tendvhienthi_create, string email_create, string madvcq_create, string madiaban_create)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                ...
                if (check_per)
                {
                    ...
                    _db.SaveChanges();
                    return RedirectToAction("Index", "DmDonvi");
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
