...
{
    public class DonViThuThapTTController : Controller
    {
        private readonly ApplicationDbContext _db;

        public DonViThuThapTTController(ApplicationDbContext db)
        {
            _db = db;
        }

        [Route("DonViThuThapTT")]
        [HttpGet]
        public IActionResult Index()
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                ...
                if (check_per)
                {
                    ...
                    ViewData["MenuLv2"] = "menu_capnhatcungcau_DonViThuThapTT";
                    return View("Views/Admin/Manages/TongHop_PhanTich_DuDoan/ThongTinCung_Cau/DonViThuThapTT/Index.cshtml", models);
                }
                else
                {
        ...
        public IActionResult Store(string madv_create, string tendv_create, string tendvhienthi_create, string email_create, string madvcq_create, string madiaban_create)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                ...
                if (check_per)
                {
                    ...
                    ViewData["MenuLv1"] = "menu_capnhatcungcau";
                    ViewData["MenuLv2"] = "menu_capnhatcungcau_DonViThuThapTT";
                    return RedirectToAction("Index", "DonViThuThapTT");
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
